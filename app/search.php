  ...
  <div class="container p-2">
    <form method="GET" class="search-article">
      <div class="search-div">
        <input class="search-input" type="text" name="name" id="name" placeholder="Naslov" autocomplete="off" />
        <input class="search-input" type="text" name="hashtag" id="hashtag" placeholder="Hashtag" />
      </div>
      <div class="filters">
        <!-- Search filter based on Category -->
        <div>
          <label for="category_select">Kategorija</label>
          <select name="category_select" id="category_select">
            <option value="">Bilo koja kategorija</option>
            <?php
      ...
      <center>
        ...
        <button type="submit" name="search" class="btn btn-primary">Pretrazi</button>
      </center>
    </form>
  </div>
</section>
...
<section class="py-1 category-list">
  <div class="container">
    <h2 class="search-heading">Rezultati pretrage :</h2>
    <div class="card-container">
      <?php
        ...
        $hashtag = "";
              ...
              // Check if we get hashtag value from URL
              if(isset($_GET['hashtag'])) {
                // Fetching hashtag value via GET and passing them to user defined 
                // function to get rid of special characters used in SQL
                $hashtag = get_safe_value($_GET['hashtag']);
              }

              // Search Query is updated with hashtag condition
              if ($hashtag != "") {
                $searchQuery .= " AND article_id IN (SELECT article_id FROM article_hashtag WHERE hashtag_name LIKE '%$hashtag%')";
              }
